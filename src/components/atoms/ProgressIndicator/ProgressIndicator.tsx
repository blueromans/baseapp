...
function MaterialCircularProgressIndicator_(
  props: MaterialCircularProgressIndicatorProps,
) {
  return (
    <Content
      {...props}
      size={useToSharedValue(props.size) as SharedValue<number>}
      value={useToSharedValue(props.value) as SharedValue<number | undefined>}
      stopped={
        useToSharedValueOptional(props.stopped, false) as SharedValue<boolean>
      }
      strokeWidth={
        useToSharedValueOptional(props.strokeWidth, 4) as SharedValue<number>
      }
      backgroundColor={
        useToSharedValue(props.backgroundColor) as SharedValue<
          string | undefined
        >
      }
      valueColor={useToSharedValue(props.valueColor) as SharedValue<string>}
    />
  );
}

function IndeterminateMaterialCircularProgressIndicator(
  ...
  return (
    <Content
      ...
      size={useToSharedValue(props.size) as SharedValue<number>}
      value={
        useSharedValue(undefined as number | undefined) as SharedValue<
          number | undefined
        >
      }
      stopped={
...
function DeterminateMaterialCircularProgressIndicator(
  props: MaterialCircularProgressIndicatorProps,
) {
  return (
    <Content
      ...
      size={useToSharedValue(props.size) as SharedValue<number>}
      value={
        useToSharedValueOptional(props.value, 0) as SharedValue<
          number | undefined
        >
      }
      strokeWidth={useToSharedValue(props.strokeWidth) as SharedValue<number>}
      backgroundColor={
        useToSharedValueOptional(
